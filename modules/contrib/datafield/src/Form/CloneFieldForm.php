 ...
 * @internal
 */
class CloneFieldForm extends FormBase {
  /**
   ...
   *   The field name.
   * @param int $delta
   *   The field delta index to clone.
   *
   * @return array
   *   The form clone.
   */
  public function buildForm(array $form, FormStateInterface $form_state, EntityInterface $entity = NULL, $field_name = NULL, $delta = 0) {
    ...
    $this->simplify($form, $form_state, $delta);
   ...
   * @param \Drupal\Core\Form\FormStateInterface $form_state
   *   The current state of the form.
   * @param int $clone_delta
   *   A delta index in items.
   */
  protected function simplify(array &$form, FormStateInterface $form_state, $clone_delta = 0) {
    $field_name = $form_state->get('field_name');
    $widget_element =& $form[$field_name]['widget'];

    if (!empty($max_delta = $widget_element["#max_delta"])) {
      $widget_element[$max_delta] = $widget_element[$clone_delta];
    }
    if (!empty($widget_element["add_more"])) {
      $widget_element["add_more"]['#access'] = FALSE;
    }

    foreach (range(0, $max_delta - 1) as $delta) {
      $children = Element::children($widget_element[$delta]);
      foreach ($children as $child) {
        if (!is_numeric($child) && $child != '_weight') {
          $widget_element[$delta][$child]['#attributes']['disabled'] = 'true';
        }
      }
    }
